...
import logging
from ..models.models import MediaUpload
...
def process_image(file_path, media_type, task_id):
    db = SessionLocal()
    try:
...
def process_video(file_path, media_type, task_id,  db:Session=Depends(get_db)):
...
def process_maintenance_check(file_path, media_type, task_id):
    db = SessionLocal()
    try:
        # Fetch task data
        task_data = fetch_task_by_id(task_id)
        ...
        db.close()
def maintenance_check_video(file_path, media_type, task_id,  db:Session=Depends(get_db)):
...
def inspection_image_analysis(image_file, prompt, media_type, task_id):
    """
...
def generate_response_video(video_file_path: str, prompt: str, media_type, task_id,  db:Session=Depends(get_db)):
    try:
        task_data = fetch_task_by_id(task_id)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
